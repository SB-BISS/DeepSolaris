...
def create_polygonset_from_image_name2(data):
    ...
    for polygons in data["polygons"]: # for each polygon entry in json
        ...
        if polygons["city"] in "Modesto": # filter for city
...
positives_Modesto = np.ndarray((0, 75, 75, 3), dtype = 'uint8')
def cut_image_and_label(file, polygons):
    global positives_Modesto
...
#    counter = 0 # initialize the counter for polygons
    for polygon in polygons: 
        ...
        positives_Modesto = np.concatenate((positives_Modesto, cropped_img))
...
negatives_Modesto = np.ndarray((0, 75, 75, 3), dtype = 'uint8')
l1 = randint(38, 112)
l2 = randint(38, 112)
def cut_image_and_label_negatives(file, coordinates):
    global negatives_Modesto
    global showcase
    ...
    for center1 in range(l1, 4970, 500):
        for center2 in range(l2, 4970, 500):
            ...
            if (overlap ==0):
                ...
                cropped_img = cropped_img.reshape((1,75,75,3))
                negatives_Modesto = np.concatenate((negatives_Modesto, cropped_img))
...
fn = (save_dir + 'positives_Modesto')
np.save(fn , positives_Modesto)    


### now saving the negative examples 

fn = (save_dir + 'negatives_Modesto')
index = np.random.choice(range(len(negatives_Modesto)), len(positives_Modesto), replace=False)
final_negatives_Modesto = negatives_Modesto[index, :, :, :]
np.save(fn , final_negatives_Modesto)    
